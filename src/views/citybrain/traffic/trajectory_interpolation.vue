...
<script>
  ...
  export default {
    ...
    methods: {
      //获取指标数据
      getIndex() {
        return axios.$get(comm.Traj_Url + 'getInterpolationIndex').then(res => {
      ...
      queryListDatas() {
        ...
        axios.$get(comm.Traj_Url + 'getInterpolationList', parmas).then(res => {
          this.datas = res;
        })
      },
      ...
      gotoTrajDetailData(val) {
        this.$router.push({name: 'trajectoryDetail', params: {index: val.index}})
      }
    }
  }
</script>
